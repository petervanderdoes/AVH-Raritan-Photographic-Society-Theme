...
if (have_posts()) {
    ...
    if (is_array($stickies)) {
        $suffusion_full_post_count_for_view = count($stickies);
    } else {
        $suffusion_full_post_count_for_view = 1;
    }
    $custom_classes = [];
    ...
    while (have_posts()) {
        ...
        ?>
        <article <?php post_class($custom_classes); ?>
            id="post-<?php the_ID(); ?>">
            <?php
    ...
    if ($number_of_cols > 0) {
        ...
        <div class='suf-tiles suf-tiles-<?php echo $number_of_cols; ?>'>
            ...
            while (have_posts()) {
                ...
                echo "\t\t<div class='suf-gradient $topmost'>" . $category_text . "</div>\n";
                $topmost = '';
                echo "\t\t<h2 class='suf-tile-title $topmost'><a class='entry-title' rel='bookmark' href='" .
                ...
                if (trim($suf_mag_excerpt_full_story_text)) {
                    ...
                    echo "\t\t<a href='" .
                         get_permalink($post->ID) .
                         "' class='suf-mag-excerpt-full-story'>$suf_mag_excerpt_full_story_text</a>";
                    echo "\t</div>\n";
                }
