<?php
/*
 * Default Events List Template This page displays a list of events, called during the em_content() if this is an events list page. You can override the default display settings pages by copying this file to yourthemefolder/plugins/events-manager/templates/ and modifying it however you need. You can display events however you wish, there are a few variables made available to you: $args - the args passed onto EM_Events::output()
 */
if (get_option('dbem_events_page_search') && ! defined('DOING_AJAX')) {
    em_locate_template('templates/events-search.php', true);
}
if ($args['category'] != 0) {
    $args['category'] = rps_EM_get_children_of_categories($args['category']);
}
...
if ($events_count > 0) {
    ...
    if (! empty($_REQUEST['action']) && $_REQUEST['action'] == 'search_events') {
        ...
        if (! empty($args['limit']) && $events_count > $args['limit']) {
            ...
            $search_args = array_merge(EM_Events::get_post_search(), array(
                'pno' => '%PAGE%',
